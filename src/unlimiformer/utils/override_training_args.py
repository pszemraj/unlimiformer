...
class TrainingOverridesArguments(Seq2SeqTrainingArguments):
    ...
    eval_steps_override: float = field(
        default=0,
        metadata={
            "help": "a fraction, to set the the save_steps w.r.t to number of steps in "
            "a single epoch. changes eval_steps. 0 to disable (default)"
        },
    )
    save_steps_override: float = field(
        default=0,
        metadata={
            "help": "a fraction, to set the the save_steps w.r.t to number of steps in "
    ...
    def __post_init__(self):
        ...
        if self.eval_steps_override > 0 or self.save_steps_override > 0:
            ...
            if (
                min(1 - diff, diff) > 1e-5
            ):  # we do it like that to support fractions modulo as well, with loss of precision
                raise ValueError(
    ...
    def effective_batch_size(self):
        if not hasattr(self, "_ebs"):
            ...
            logger.warning(
                f"Training with {self.per_device_train_batch_size} per_device_train_size, {self.n_gpu} gpus and "
    ...
    def apply_overrides(self, dataset_size):
        ...
        if eval_steps >= self.logging_steps:
            if eval_steps % self.logging_steps != 0:
                logger.warning(
                    f"Eval steps override would result in eval every {eval_steps} steps, but it is not a "
                ...
                eval_steps = (
                    eval_steps + self.logging_steps - eval_steps % self.logging_steps
                )
        ...
        if save_steps < eval_steps or save_steps % eval_steps != 0:
            logger.warning(
                f"Save steps override would result in eval every {save_steps} steps, but it is not a "
        ...
        logger.warning(
            f"Using overrides with dataset of size {dataset_size} and effective batch size of "
