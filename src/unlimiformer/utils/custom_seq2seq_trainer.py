...
class CustomTrainer(Seq2SeqTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        self.mock_predictions_to_assign_zero_metric_score = self.tokenizer.encode(
            "TOO_MANY_INPUT_TOKENS", return_tensors="np"
        )[0]
    ...
    def prediction_step(
        ...
        if not ("labels" in inputs or "decoder_input_ids" in inputs):
            if model.training:
                logger.warning(
                    "When computing loss, must give labels or decoder_input_ids. "
        ...
        if (
            has_labels
        ):  # changed the order of the if's here because there is no point going through the model if there are no labels to compute the loss on..
            with torch.no_grad():
    ...
    def _post_process_function(self, untokenized_eval_dataset, predictions):
        ...
        for i, (instance, not_valid_for_eval) in enumerate(
            zip(untokenized_eval_dataset, self.eval_dataset["not_valid_for_eval"])
        ):
            if not_valid_for_eval:
                id_to_prediction[
                    instance["id"]
                ] = self.mock_predictions_to_assign_zero_metric_score
            else:
    ...
    def evaluate(
        ...
        **gen_kwargs,
        ...
        untokenized_eval_dataset = (
            self._untokenized_eval_dataset
            if untokenized_eval_dataset is None
            else untokenized_eval_dataset
        )
        ...
        if output.predictions is not None:
            eval_preds = self._post_process_function(
                untokenized_eval_dataset, output.predictions
            )
            ...
            if self._output_dir is not None and self.is_world_process_zero():
                ...
                output_prediction_file = os.path.join(
                    self._output_dir,
                    f"generated_predictions_eval_{self.state.global_step}.json",
                )
                with open(output_prediction_file, "w") as writer:
                    json.dump(predictions, writer, indent=4)

                output_labels_file = os.path.join(self._output_dir, f"eval_labels.json")
