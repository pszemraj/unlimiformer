...
class RandomTrainingUnlimiformer(Unlimiformer[ModelType]):
    def __init__(
        self, model: ModelType, random_knn_initial_inputs=False, *args, **kwargs
    ):
    ...
    def inject_training_hooks(self, model):
        ...
        decoder_layers_to_run = self.attention_layer_to_run(
            self.knn_layer_begin, self.knn_layer_end
        )
        ...
        for decoder_layer in decoder_layers_to_run:
            ...
            attention.forward = self.create_self_attn_random_pre_forward_hook(
                attention.forward
            )
        ...
        for decoder_layer in decoder_layers_to_run:
            ...
            decoder_layer.forward = self.create_decoder_layer_random_func(
    ...
    def create_decoder_layer_random_func(
        ...
        def checkpointed_decoder_layer(
            ...
            def sample_and_forward(
                ...
                sampled_input, _ = self.sample_long_input(
                    long_inputs, long_inputs_mask, rand_indices
                )
    ...
    def sample_random_indices(self):
        ...
        for seq_len in seq_lens:
            if seq_len < self.actual_model_window_size:
                rand_indices = torch.arange(self.actual_model_window_size).to(
                    self.device
                )
                rand_indices_list.append(rand_indices)
                continue

            rand_indices = torch.torch.randperm(seq_len)[
                : self.actual_model_window_size
            ].to(self.device)
            if seq_len < self.actual_model_window_size:
                padding = max(self.actual_model_window_size - seq_len, 0)
                rand_indices = torch.cat(
                    [rand_indices, torch.arange(padding).to(self.device) + seq_len],
                    axis=-1,
                ).to(self.device)
            rand_indices_list.append(rand_indices)
    ...
    def random_inputs_forward_hook(
        self, input_ids=None, attention_mask=None, labels=None, **kwargs
    ):
        self.model.base_model.decoder.gradient_checkpointing = False
        self.long_inputs_encoded, self.long_inputs_mask = self.chunked_encode_input(
            input_ids=input_ids, attention_mask=attention_mask
        )

        #  TODO: should the inputs be sampled or the truncated beginning?
        if self.random_knn_initial_inputs:
            encoded_inputs, encoded_inputs_mask = self.sample_long_input(
                self.long_inputs_encoded, self.long_inputs_mask
            )
        else:
            encoded_inputs = self.long_inputs_encoded[
                :, : self.actual_model_window_size
            ]
            encoded_inputs_mask = self.long_inputs_mask[
                :, : self.actual_model_window_size
            ]
        return self.original_forward_func(
            encoder_outputs=(encoded_inputs,),
            labels=labels,
            attention_mask=encoded_inputs_mask,
            **kwargs,
        )

    def sample_long_input(
        self, long_inputs_encoded, long_inputs_mask, random_indices=None
    ):
        ...
        random_mask = (
            torch.zeros_like(long_inputs_mask)
            .to(self.device)
            .scatter_(dim=-1, index=random_indices, src=torch.ones_like(random_indices))
            .bool()
            .to(self.device)
        )
        sampled_input = (
            long_inputs_encoded[random_mask]
            .reshape(batch_size, self.actual_model_window_size, -1)
            .to(self.device)
        )
        sampled_mask = (
            long_inputs_mask[random_mask]
            .reshape(batch_size, self.actual_model_window_size)
            .to(self.device)
        )
