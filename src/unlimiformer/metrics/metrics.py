    ...
    def _compute_metrics(self, id_to_pred, id_to_labels):
        ...
        for metric in self._metrics:
            ...
            if metric.requires_decoded:
                ...
                result = metric(
                    id_to_pred_decoded, id_to_labels_decoded, is_decoded=True
                )
            else:
        ...
        results["mean_prediction_length_characters"] = np.mean(
            [len(pred) for pred in id_to_pred_decoded.values()]
        )

        elem = next(iter(id_to_pred.values()))
        if not (
            (isinstance(elem, list) and isinstance(elem[0], str))
            or isinstance(elem, str)
        ):
            tokenizer = self._kwargs["tokenizer"]
            results["mean_prediction_length_tokens"] = np.mean(
                [
                    np.count_nonzero(np.array(pred) != tokenizer.pad_token_id)
                    for pred in id_to_pred.values()
                ]
...
def load_metric(paths: List[str], **kwargs):
    ...
    if len(scrolls_custom_metrics) > 0:
        scrolls_custom_metrics.insert(
            0, ""
        )  # In order to have an identifying comma in the beginning
        metric_cls_list.append(
            ScrollsWrapper(",".join(scrolls_custom_metrics), **kwargs)
        )
...
class ScrollsWrapper(Metric):
    def __init__(self, comma_separated_metric_names, **kwargs) -> None:
        ...
        self._metric = hf_load_metric(
            download_metric(), comma_separated_metric_names, keep_in_memory=True
        )
    ...
    def _compute_metrics(self, id_to_pred, id_to_labels) -> Dict[str, float]:
        return self._metric.compute(
            **self._metric.convert_from_map_format(id_to_pred, id_to_labels)
        )
...
class HFMetricWrapper(Metric):
    ...
    def _compute_metrics(self, id_to_pred, id_to_labels) -> Dict[str, float]:
        return self._metric.compute(
            **self.convert_from_map_format(id_to_pred, id_to_labels), **self.kwargs
        )
