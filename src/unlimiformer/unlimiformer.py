...
from unlimiformer.index_building import Datastore, DatastoreBatch
...
class Unlimiformer(Generic[ModelType]):
    ...
    def break_into(self, model):
        ...
        if self.specific_head is None:
            self.head_nums = (
                Ellipsis  # torch.arange(0, self.num_heads, device=self.device)
            )
        else:
    ...
    def inject_hooks(self, model):
        ...
        attention_layers_to_run = self.attention_op_to_run(
            self.layer_begin, self.layer_end
        )
        for layer in attention_layers_to_run:
            self.register_hook(layer, self.attention_forward_hook)

        decoder_layers_to_run = self.attention_layer_to_run(
            self.layer_begin, self.layer_end
        )
        self.original_decoder_layer_cross_attn_forward_funcs = []
        for i, decoder_layer in enumerate(decoder_layers_to_run):
            self.original_decoder_layer_cross_attn_forward_funcs.append(
                self.cross_attention(decoder_layer).forward
            )
            self.cross_attention(
                decoder_layer
            ).forward = self.create_cross_attn_pre_forward_hook(
                self.cross_attention(decoder_layer).forward, decoder_layer, i
            )
    ...
    def inject_training_hooks(self, model):
        ...
        for decoder_layer in decoder_layers_to_run:
            ...
            self.original_decoder_layer_self_attn_forward_funcs.append(
                attention.forward
            )
            attention.forward = self.create_self_attn_pre_forward_hook(
                attention.forward
            )
        ...
        for i, decoder_layer in enumerate(decoder_layers_to_run):
            ...
            attention.forward = self.create_cross_attn_pre_forward_hook(
                attention.forward, decoder_layer, i
            )

        self.original_decoder_layer_forward_funcs = []
        for decoder_layer in decoder_layers_to_run:
            self.original_decoder_layer_forward_funcs.append(decoder_layer.forward)
            decoder_layer.forward = self.create_decoder_layer_func(
    ...
    def inject_hooks_for_unaffected_layers(self, model, decoder_layers_to_run):
        ...
        for decoder_layer in non_injected_decoder_layers:
            ...
            decoder_layer.forward = self.create_noninjected_decoder_layer_func(
                decoder_layer.forward, decoder_layer
            )
    ...
    def create_noninjected_decoder_layer_func(
        self, decoder_layer_original_forward_func, decoder_layer
    ):
        def checkpointed_decoder_layer(
            hidden_states: torch.Tensor,
    ...
    def break_out(self, model):
        ...
        for decoder_layer, original_func in zip(
            decoder_layers_to_run, self.original_decoder_layer_cross_attn_forward_funcs
        ):
    ...
    def remove_training_hooks(self, model):
        ...
        for decoder_layer, original_func in zip(
            decoder_layers_to_run, self.original_decoder_layer_self_attn_forward_funcs
        ):
        ...
        for decoder_layer, original_func in zip(
            ...
            self.cross_attention(decoder_layer).forward = original_func
        for decoder_layer, original_func in zip(
            decoder_layers_to_run, self.original_decoder_layer_forward_funcs
        ):
    ...
    def reset_memory(self, input_ids, attention_mask):
        ...
        self.prev_tokens = [
            None
            for _ in range(len(self.original_decoder_layer_cross_attn_forward_funcs))
        ]
        self.last_beam_idx = None
        ...
        if self.save_heatmap:
            if self.heatmap is not None:
                print(
                    f"Generated: {self.tokenizer.decode(self.generated_input_ids[0])}"
                )
        ...
        for (
            ...
            if (not self.use_datastore) or self.test_datastore:
                ...
                key = [
                    layer[0][:, :, update_start_ind:update_end_ind]
                    for layer in layers_kv
                ]
                value = [
                    layer[1][:, :, update_start_ind:update_end_ind]
                    for layer in layers_kv
                ]
                ...
                value = torch.stack(
                    value, dim=0
                )  # (num_layers, batch, head, time, dim)

                self.prompt_keys.append(
                    key
                )  # (num_layers, batch, head, chunked_source_len, dim)
                self.prompt_values.append(
                    value
                )  # (num_layers, batch, head, chunked_source_len, dim)
                self.prompt_attention_mask.append(
                    chunk_attention_mask
                )  # (batch, chunked_source_len)
        ...
        if (not self.use_datastore) or self.test_datastore:
            self.prompt_keys = torch.cat(
                self.prompt_keys, dim=-2
            )  # (num_layers, batch, head, source_len, dim)
            self.prompt_values = torch.cat(
                self.prompt_values, dim=-2
            )  # (num_layers, batch, head, source_len, dim)
            self.prompt_attention_mask = torch.cat(
                self.prompt_attention_mask, dim=-1
            )  # (batch, source_len)
    ...
    def chunked_encode_input(self, input_ids, attention_mask):
        ...
        for (
            ...
            chunk = input_ids[:, context_start_ind:context_end_ind]
            chunk_attention_mask = attention_mask[:, context_start_ind:context_end_ind]
            output = self.model.base_model.encoder(
                chunk,
                attention_mask=chunk_attention_mask,
                return_dict=True,
            ...
            encoder_last_hidden_state = encoder_last_hidden_state[
                :, update_start_ind:update_end_ind
            ]  # (batch, chunked_time, dim)
            chunk_attention_mask = chunk_attention_mask[
                :, update_start_ind:update_end_ind
            ]  # (batch, chunked_time)

            long_inputs_encoded.append(
                encoder_last_hidden_state
            )  # (batch, chunked_source_len, dim)
            long_inputs_mask.append(chunk_attention_mask)  # (batch, chunked_source_len)

        long_inputs_encoded = torch.cat(
            long_inputs_encoded, dim=1
        )  # (batch, source_len, dim)
    ...
    def window_indices(self, total_seq_len):
        ...
        if total_seq_len <= self.model_encoder_max_len:
            ...
            results.append(
                (context_start, context_end, update_start_ind, update_end_ind)
            )
            ...
            while context_end < total_seq_len:
                ...
                context_start = (
                    ...
                    if context_end < total_seq_len
                    else total_seq_len - self.model_encoder_max_len
                )
                update_start_ind = max(update_start_ind + stride, update_end_ind)
                # last window always should update until the end
                update_end_ind = (
                    min(total_seq_len, update_end_ind + stride)
                    if context_end < total_seq_len
                    else total_seq_len
                )
    ...
    def pre_generate_hook(self, input_ids, **kwargs):
        ...
        if "attention_mask" in kwargs:
            ...
            new_kwargs["attention_mask"] = kwargs["attention_mask"][
                :, : self.actual_model_window_size
            ]
        new_kwargs["use_cache"] = True
        return self.original_generate_func(
            input_ids[:, : self.actual_model_window_size], **new_kwargs
        )
        ...
        if not self.is_input_encoding_pass:
            if self.model.training:
                ...
                (
                    self.long_inputs_encoded,
                    self.long_inputs_mask,
                ) = self.chunked_encode_input(
                    input_ids=input_ids, attention_mask=attention_mask
                )
                input_ids = input_ids[:, : self.actual_model_window_size]
                attention_mask = (
                    attention_mask[:, : self.actual_model_window_size]
                ...
                if kwargs.get("decoder_input_ids") is not None:
                    self.generated_input_ids = torch.cat(
                        [self.generated_input_ids, kwargs["decoder_input_ids"]], axis=-1
                    )

        result = self.original_forward_func(
            input_ids=input_ids, labels=labels, attention_mask=attention_mask, **kwargs
        )
        self.is_first_test_decoding_step = False
        return result

    def create_cross_attn_pre_forward_hook(
        self, original_cross_attn_forward_func, decoder_layer, i
    ):
        def attention_pre_forward_hook(
            hidden_states, attention_mask=None, *args, **kwargs
        ):
            ...
            if self.model.training:
                ...
                (
                    ...
                    past_key_value,
                ) = original_cross_attn_forward_func(
                    hidden_states=hidden_states,
                    attention_mask=attention_mask,
                    *args,
                    **kwargs,
                ...
                return original_cross_attn_forward_func(
                    hidden_states=hidden_states,
                    attention_mask=attention_mask,
                    *args,
                    **kwargs,
    ...
    def attention_forward_hook(self, module, input, output):
        ...
        with torch.no_grad():
            ...
            if self.use_datastore:
                ...
                # k_proj in attention?
                attention_layer_list = self.attention_layer_to_capture(
                    self.layer_begin, self.layer_end
                )
                ...
                k_proj = k_proj.view(
                    1, self.num_heads, query.shape[-1], k_proj.shape[0]
                )  # (1, num_heads, attn_dim, embed_dim)
                datastore_query = query.unsqueeze(
                    -2
                )  # (batch * beam, num_heads, 1, attn_dim)
                datastore_query = torch.matmul(
                    datastore_query, k_proj
                )  # (batch * beam, num_heads, 1, embed_dim)
                datastore_query = datastore_query.squeeze(
                    -2
                )  # (batch * beam, num_heads, embed_dim)
                datastore_query = datastore_query.view(
                    (self.datastore.batch_size, -1, datastore_query.shape[2])
                )  # (batch, beam * num_heads, embed_dim)
                # then search
                if self.reconstruct_embeddings:
                    # embeddings: (batch, beam * head, actual_model_window_size, dim)
                    (
                        top_search_key_scores,
                        top_search_key_indices,
                        embeddings,
                    ) = self.datastore.search_and_reconstruct(
                        datastore_query, k=self.actual_model_window_size
                    )
                else:
                    (
                        top_search_key_scores,
                    ...
                    embeddings = torch.take_along_dim(
                        input=self.embeddings.unsqueeze(1),
                ...
                top_search_key_scores = top_search_key_scores.reshape(
                    (
                        self.datastore.batch_size,
                        -1,
                        self.num_heads,
                        self.actual_model_window_size,
                    )
            ...
            if (not self.use_datastore) or self.test_datastore:
                ...
                this_layer_prompt_values = self.prompt_values[
                    self.cur_decoder_layer_index
                ]
                # query: (batch * beam, head, dim)
                ...
                prompt_attention_mask_to_add = prompt_attention_mask_to_add.unsqueeze(
                    1
                ).unsqueeze(1)
                attn_weights += (
                    prompt_attention_mask_to_add  # (batch, beam, head, source_len)
                )
                if (
                    self.exclude_attention
                    and attn_weights.shape[-1] > self.actual_model_window_size
                ):
                ...
                top_key_scores, top_key_indices = torch.topk(
                    attn_weights, k=topk, dim=-1, sorted=True
                )  # (batch, beam, head, trunc_source)
                if self.save_heatmap:
                    # heatrow: (beam, heads, source_len)
                    heatrow = torch.zeros(
                        [
                            top_key_indices.shape[1],
                            top_key_indices.shape[2],
                            this_layer_prompt_keys.shape[-2],
                        ],
                        dtype=torch.float,
                    )
                    heatrow = heatrow.scatter(
                        index=top_key_indices[0],
                        src=torch.ones_like(top_key_scores[0]),
                        dim=-1,
                    )
                    # heatrow = torch.nn.functional.softmax(heatrow, dim=-1)
                    # self.heatmap: (beam, heads, targets, source_len)
                    self.heatmap = torch.cat(
                        [self.heatmap, heatrow.unsqueeze(-2)], axis=-2
                    )

            if self.test_datastore:
                assert top_key_indices.shape == top_search_key_indices.shape
                assert (
                    torch.mean((top_key_indices == top_search_key_indices).float())
                    > 0.99
                )
        ...
        if self.use_datastore:
            ...
            k_weight = k_proj_layer.weight.view(
                1, 1, self.num_heads, embed_dim // self.num_heads, embed_dim
            ).transpose(
                -2, -1
            )  # (1, 1, heads, embed_dim, attn_dim)
            k_bias = k_proj_layer.bias.view(
                1, self.num_heads, embed_dim // self.num_heads
            ).unsqueeze(-2)
            v_weight = v_proj_layer.weight.view(
                1, 1, self.num_heads, embed_dim // self.num_heads, embed_dim
            ).transpose(
                -2, -1
            )  # (1, heads, embed_dim, attn_dim)
            v_bias = v_proj_layer.bias.view(
                1, self.num_heads, embed_dim // self.num_heads
            ).unsqueeze(-2)
            # new_keys, new_values: (batch, beam, head, encoder_len, attn_dim)
            new_keys = torch.matmul(embeddings, k_weight) + k_bias.unsqueeze(
                0
            )  # (beam, head, encoder_len, embed_dim)
            new_values = torch.matmul(embeddings, v_weight) + v_bias.unsqueeze(
                0
            )  # (beam, head, encoder_len, embed_dim)
        else:
            # this_layer_prompt_keys:   (batch,       head, source_len, dim)
            # top_key_indices:          (batch, beam, head, trunc_source)
            new_keys = torch.take_along_dim(
                this_layer_prompt_keys.unsqueeze(1),
                indices=top_key_indices.unsqueeze(-1),
                dim=-2,
            )  # (batch, head, trunc_source, attn_dim)
            new_values = torch.take_along_dim(
                this_layer_prompt_values.unsqueeze(1),
                indices=top_key_indices.unsqueeze(-1),
                dim=-2,
            )  # (batch, head, trunc_source, attn_dim)

        if self.test_datastore:
            correct_keys = torch.take_along_dim(
                this_layer_prompt_keys.unsqueeze(1),
                indices=top_key_indices.unsqueeze(-1),
                dim=-2,
            )  # (batch, head, trunc_source, attn_dim)
            correct_values = torch.take_along_dim(
                this_layer_prompt_values.unsqueeze(1),
                indices=top_key_indices.unsqueeze(-1),
                dim=-2,
            )  # (batch, head, trunc_source, attn_dim)
            ...
            assert (
                torch.mean(
                    torch.isclose(correct_keys, new_keys, rtol=1e-3, atol=1e-3).float()
                )
                > 0.99
            )
            assert (
                torch.mean(
                    torch.isclose(
                        correct_values, new_values, rtol=1e-3, atol=1e-3
                    ).float()
                )
                > 0.99
            )
    ...
    def train_attention_forward_hook(self, module, input, output):
        ...
        with torch.no_grad():
            ...
            topk = min(self.actual_model_window_size, attn_weights.shape[-1])
            top_key_scores, top_key_indices = torch.topk(
                attn_weights, k=min(topk, attn_weights.shape[-1]), dim=-1, sorted=True
            )  # (batch, beam, head, tgt, trunc_source)

        # this_layer_prompt_keys:   (batch,          head,    source_len, dim)
        # top_key_indices:          (batch, tgt_len, head, 1, trunc_source)
        new_keys = torch.take_along_dim(
            this_layer_prompt_keys.unsqueeze(2).unsqueeze(1),
            indices=top_key_indices.unsqueeze(-1),
            dim=-2,
        )  # (batch, tgt_len, head, 1, trunc_source, attn_dim)
        new_values = torch.take_along_dim(
            this_layer_prompt_values.unsqueeze(2).unsqueeze(1),
            indices=top_key_indices.unsqueeze(-1),
            dim=-2,
        )  # (batch, tgt_len, head, 1, trunc_source, attn_dim)
    ...
    def reorder_cache_hook(self, past, beam_idx):
        ...
        for i, layer_prev_tokens in enumerate(self.prev_tokens):
            if layer_prev_tokens is not None:
                self.prev_tokens[i] = layer_prev_tokens.flatten(0, 1)[beam_idx].reshape(
                    layer_prev_tokens.shape
                )
    ...
    def plot_heatmap(self, data, xticklabels="auto", yticklabels="auto"):
        ...
        for i in range(data.shape[0]):
            ...
            cur_ax = axes
            axes.set_title(
                f"Head #{i}, length: {data.shape[2]}, target length: {data.shape[1]}"
            )
            cur_ax = axes
...
class UnlimiformerBART(Unlimiformer[BartModel]):
    ...
    def create_key_value(self, encoder_hidden_states, decoder_layer):
        ...
        value = (
            value.view(value.shape[0], -1, attention.num_heads, attention.head_dim)
            .transpose(1, 2)
            .contiguous()
        )
        # key, value: (batch, heads, time, attn_dim)
...
class UnlimiformerT5(Unlimiformer[T5Model]):
    ...
    def create_key_value(self, encoder_hidden_states, decoder_layer):
        ...
        value = (
            value.view(
                value.shape[0], -1, attention.n_heads, attention.key_value_proj_dim
            )
            .transpose(1, 2)
            .contiguous()
        )
